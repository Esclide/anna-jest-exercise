...
describe('Lesson page: Completing the lesson', () => {
  ...
  it('Step 1. Navigate to home page and click on selected lesson', async () => {
  ...
  it('Step 2. Answer the first question', async () => {
    await LessonPage.elements.testTitle.waitFor();

    expect(await LessonPage.getQuestionNumber()).to.equal(1);
    expect(await LessonPage.getQuestionText()).to.equal('Energy cones from calories');
    ...
    expect(await LessonPage.elements.questionField.continueButton.getText()).to.equal('CONTINUE')
    await LessonPage.pressContinueButton();
  }, jestTimeout)

  it('Step 3. Answer the second question', async () => {
    expect(await LessonPage.getQuestionNumber()).to.equal(2);
    expect(await LessonPage.getQuestionText()).to.equal('A healthy diet is about _____. Select three');
    ...
    expect(await LessonPage.elements.questionField.continueButton.getText()).to.equal('CHECK ANSWER');
    await LessonPage.pressContinueButton();
  }, jestTimeout)


  it('Step 3. Return to home page', async () => {
